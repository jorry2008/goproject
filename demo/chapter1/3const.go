...
// VAR_GLASS 常量是一个超级简单值的标识符，唯一的特性就是：在程序运行时，不会被修改的量
// 常量中的数据类型只可以是 布尔型、数字型（整数型、浮点型和复数）和字符串型 这种非引用的基本类型（所以，常量的类型，仍然属于基本类型，可以参与计算）
// 与变量相比，差异：常量声明了必须要初始化赋值！表达式赋值必须使用内置函数！
